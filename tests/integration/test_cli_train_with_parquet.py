"""Integration tests for MLM training with Parquet data files."""
...
def _write_parquet(path: Path, n_rows: int, seq_min_len: int, seq_max_len: int):
    """Write a Parquet file with protein sequences for MLM training."""
...
def test_cli_train_with_parquet_e2e(tmp_path):
    """Test MLM training with Parquet data files."""
    runner = CliRunner()
