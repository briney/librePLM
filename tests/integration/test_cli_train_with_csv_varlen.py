"""Integration tests for MLM training with variable length CSV data."""
...
def _write_csv_varlen(path: Path, n_rows: int, seq_min_len: int, seq_max_len: int):
    """Write a CSV file with variable length protein sequences."""
...
def test_cli_train_with_csv_varlen(tmp_path):
    """Test MLM training with variable length sequences in CSV files."""
    runner = CliRunner()
    ...
    _write_csv_varlen(train_csv, n_rows=8, seq_min_len=12, seq_max_len=28)
    _write_csv_varlen(eval_csv, n_rows=4, seq_min_len=12, seq_max_len=28)
